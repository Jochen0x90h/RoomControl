...
public:
	 ...
	 */
	Data const *get(uint8_t deviceId) const;
	 ...
	 */
	void set(uint8_t deviceId, Data &data);
	 ...
	 */
	int getSubscriberCount(uint8_t deviceId, uint8_t command);
	 ...
	 * @param command command: 0: deactivate, 1: activate
	 */
	void test(uint8_t deviceId, uint8_t command);
...
protected:
	...
	// find alarm by id
	Alarm *findAlarm(uint8_t deviceId) const;
