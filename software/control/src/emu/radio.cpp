...
void doGui(Gui &gui, int &id) {
	...
	if (rocker != -1) {
		...
		if (ms > 3000 && rocker == 0) {
			...
			encrypt(message, radio::deviceId, radio::deviceId, header, headerLength, message, 16, 4, defaultAesKey);
			...
			encrypt(message, radio::deviceId, radio::counter, header, headerLength, message, 0, 4, aesKey);
