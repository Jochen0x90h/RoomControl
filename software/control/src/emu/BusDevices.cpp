...
Array<DeviceId> BusDevices::getBusDevices() {
	return deviceIds;
}

Array<EndpointType> BusDevices::getBusDeviceEndpoints(DeviceId deviceId) {
	switch (deviceId) {
	case 0x00000001:
...
void BusDevices::subscribeBus(uint8_t &endpointId, DeviceId deviceId, uint8_t endpointIndex) {
	int stateIndex = 0;
...
void BusDevices::unsubscribeBus(uint8_t &endpointId) {
	endpointId = 0;
}

void BusDevices::busSend(uint8_t endpointId, const uint8_t *data, int length) {
