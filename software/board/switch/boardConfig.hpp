...
//#include <stm32f0/SpiMasterSingle16.hpp>
...
/*
	SpiMasterSingle16 relayDriver{
		gpio::PA(5), // sck
		gpio::PA(7), // mosi
		gpio::PA(6), // miso
		gpio::PA(3)}; // cs
*/
	BusNodeImpl busNode{gpio::PA(10), gpio::PA(9)};

	FlashImpl storageFlash{FLASH_ADDRESS + 24 * 1024, 2, 1024};
	FlashStorage storage{storageFlash};

	FlashImpl countersFlash{FLASH_ADDRESS + 24 * 1024 + 2048, 10, 1024};
	FlashStorage counters{countersFlash};
};

struct DriversFlashTest {
	FlashImpl flash{FLASH_ADDRESS + 24 * 1024, 2, 1024};
};

struct DriversStorageTest {
	FlashImpl flash{FLASH_ADDRESS + 24 * 1024, 2, 4096};
...
struct DriversSpiMasterTest {
	SpiMasterImpl spi{1,
		...
		gpio::PA(7), // mosi
		gpio::PA(6)}; // miso
	SpiMasterImpl::Channel transfer{spi, gpio::PA(3)};
	SpiMasterImpl::Channel command{spi, gpio::PA(4)};
	SpiMasterImpl::Channel data{spi, gpio::PA(4)};
/*
	SpiMasterSingle16 transfer{
		gpio::PA(5), // sck
		gpio::PA(7), // mosi
		gpio::PA(6), // miso
		gpio::PA(3)}; // cs*/
